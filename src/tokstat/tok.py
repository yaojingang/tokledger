...
def _run_tokstat(args: list[str]) -> int:
    return subprocess.run(_tokstat_command(args), check=False).returncode


def _tokstat_command(args: list[str]) -> list[str]:
    command = [sys.executable, "-m", "tokstat.cli", "--db", str(_db_path())]
    timezone = os.environ.get("TOKSTAT_TIMEZONE")
    if timezone:
...
def _db_path() -> Path:
    return Path(os.environ.get("TOKSTAT_DB_PATH", Path.home() / ".tokstat/usage.sqlite")).expanduser()


def _report_dir() -> Path:
    return Path(os.environ.get("TOKSTAT_REPORT_DIR", Path.home() / ".tokstat/reports")).expanduser()
